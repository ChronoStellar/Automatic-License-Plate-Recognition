...
model = YOLO('./tracking_model.pt')  # Replace it accordingly if using a custom-trained model
...
while True:
    ...
    for result in results:  # Each result corresponds to a frame
        ...
        for box in boxes:
            ...
            # Display the cropped (zoomed) object (uncomment bellow)
            # cv2.imshow('Cropped Object Detection', cropped_object)
            
            #Draw bounding boxes and labels on the original frame for reference
